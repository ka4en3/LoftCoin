...
public class BalanceFormatter implements Formatter<Wallet> {
    ...
    Place BalanceFormatter class in Component as is (because not binding interface or abstract to some custom realization)*/
    /*@Inject is enough*/
    @Inject
    BalanceFormatter(Context context) {
        this.context = context;
    }

    @NonNull
    @Override
    public String format(@NonNull Wallet wallet) {
        ...
        symbols.setCurrencySymbol(wallet.coin().symbol());
        format.setDecimalFormatSymbols(symbols);
        return format.format(wallet.balance());
    }
